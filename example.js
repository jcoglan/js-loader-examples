...
Loader = {
  ...
  external: {
    ...
    labjs: function(path, callback) {
      $LAB.script(path).wait(callback);
    },
    packages: function(path, callback) {
      JS.Package.Loader.loadFile(path, callback);
    },
    ...
    yepnope: function(path, callback) {
      yepnope({load: [path], callback: callback});
    }
  }
};
