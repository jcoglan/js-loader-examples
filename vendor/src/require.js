    ...
    function configurePackageDir(pkgs, currentPackages, dir) {
        ...
        (pkgObj = currentPackages[i]); i++) {
            ...
            pkgs[pkgObj.name] = {
                ...
                main: (pkgObj.main || "lib/main").replace(currDirRegExp, '').replace(jsSuffixRegExp, '')
            };
        }
    }
...
/*
        ...
        function execManager(manager) {
            var i, ret, waitingCallbacks, cb = manager.callback,
        ...
        context = {
            ...
            require: function(deps, callback, relModuleMap) {
                ...
                if (typeof deps === "string") {
                    ...
                    if (ret === undefined) {
                        return req.onError(new Error("require: module name '" + moduleMap.fullName + "' has not been loaded yet for context: " + contextName));
                    }
                    return ret;
                }
            ...
            completeLoad: function(moduleName) {
                ...
                if (args) {
                    ...
                    callDefMain([moduleName, [], moduleName === "jquery" && typeof jQuery !== "undefined" ?
                    function() {
                        return jQuery;
                    } : null]);
                }
            ...
            nameToUrl: function(moduleName, ext, relModuleMap) {
                ...
                if (moduleName.indexOf("./") === 0 || moduleName.indexOf("../") === 0) {
                    ...
                    trimDots(syms);
                    url = syms.join('/') + (ext ? ext : (req.jsExtRegExp.test(moduleName) ? "" : ".js"));
                } else {
    ...
    req.onScriptLoad = function(evt) {
        ...
        var node = evt.currentTarget || evt.srcElement,
            contextName, moduleName, context;
    ...
    req.callReady = function() {
        var callbacks = s.readyCalls,
            i, callback, contexts, context, prop;
        if (s.isPageLoaded && s.isDone) {
            if (callbacks.length) {
                s.readyCalls = [];
                for (i = 0;
                (callback = callbacks[i]); i++) {
                    callback();
                }
            }
